...
for /d %%D in ("%LocalAppData%\Programs\Python\Python3*") do (
    if exist "%%D\python.exe" (
        "%%D\python.exe" -c "import sys; raise SystemExit(0 if sys.version_info[:2] in [(3,10),(3,11),(3,12),(3,13)] else 1)" >nul 2>nul
        if not errorlevel 1 (
...
echo Nie znaleziono zgodnego Pythona 3.10-3.13 ani lokalnego Pythona aplikacji.
